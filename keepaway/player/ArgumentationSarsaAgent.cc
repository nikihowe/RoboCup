...
#include <time.h>
#include <sstream>
#include "ArgumentationSarsaAgent.h"
...
ArgumentationSarsaAgent::ArgumentationSarsaAgent( int numFeatures, int numActions, bool bLearn,
                                    double widths[],
...
double ArgumentationSarsaAgent::getQ(int action) {
...
void ArgumentationSarsaAgent::setEpsilon(double epsilon) {
  this->epsilon = epsilon;
}

double ArgumentationSarsaAgent::getPotential(
...
ArgumentationSarsaAgent::getSituation(
        double state[]) {
...
double ArgumentationSarsaAgent::getG(Situation sit, Value val) {
    switch (sit) {
        case Safe:
...
bool ArgumentationSarsaAgent::checkOpen(double state[], int i) {
    if (i == 1) {
        return state[11] > 15;
    } else if (i == 2) {
        return state[12] > 15;
...
bool ArgumentationSarsaAgent::checkFar(double state[], int i) {
    if (i == 1) {
        return state[9] > 10;
    } else if (i == 2) {
        return state[10] > 10;
...
int ArgumentationSarsaAgent::startEpisode( double state[] )
{
...
int ArgumentationSarsaAgent::step( double reward, double state[] )
{
  // Niki-written
...
void ArgumentationSarsaAgent::endEpisode( double reward )
{
  if (hiveMind) loadColTabHeader(colTab, weights);
...
void ArgumentationSarsaAgent::shutDown()
{
...
int ArgumentationSarsaAgent::selectAction()
{
  int action;
...
bool ArgumentationSarsaAgent::loadWeights( char *filename )
{
...
bool ArgumentationSarsaAgent::saveWeights( char *filename )
{
  if (hiveMind) {
...
double ArgumentationSarsaAgent::computeQ( int a )
{
  double q = 0;
  for ( int j = 0; j < numTilings; j++ ) {
    q += weights[ tiles[ a ][ j ] ];
...
int ArgumentationSarsaAgent::argmaxQ()
{
  int bestAction = 0;
...
void ArgumentationSarsaAgent::updateWeights( double delta )
{
...
void ArgumentationSarsaAgent::loadTiles( double state[] )
{
...
void ArgumentationSarsaAgent::clearTrace( int f)
{
  if ( f > RL_MEMORY_SIZE || f < 0 )
...
void ArgumentationSarsaAgent::clearExistentTrace( int f, int loc )
{
  if ( f > RL_MEMORY_SIZE || f < 0 )
...
void ArgumentationSarsaAgent::decayTraces( double decayRate )
{
  int f;
...
void ArgumentationSarsaAgent::setTrace( int f, float newTraceValue )
{
  if ( f > RL_MEMORY_SIZE || f < 0 )
...
void ArgumentationSarsaAgent::increaseMinTrace()
{
  minimumTrace *= 1.1;
...
void ArgumentationSarsaAgent::setParams(int iCutoffEpisodes, int iStopLearningEpisodes)
{

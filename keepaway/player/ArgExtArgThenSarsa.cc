...
#define START_LEARNING_TIME 500 // when to transition from framework to sarsa
...
public:
  ...
  int getFrameworkRecommendedAction(double state[]);
...
// This is what we use in look-ahead advice
...
double ArgumentationAgent::getPotential(double state[], int action) {
    ...
    // Fast decision-tree approach below
    //std::set< std::set<Argument> > prefExts = getPreferredExtensionsFast(state, args);
...
// Use the argumentation framework to get a single recommended action
int ArgumentationAgent::getFrameworkRecommendedAction(double state[]) {
...
int ArgumentationAgent::startEpisode( double state[] )
{
  ...
  if (episodeCount < START_LEARNING_TIME) {
    lastAction = getFrameworkRecommendedAction(state);
  } else {

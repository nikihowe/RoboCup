 ...
 * NOTE: numK = 5, numT = 4
  ...
  for ( int i = 1; i < numK; i++ )
    state[ j++ ] = WB_dist_to_K[ i ]; // 1,2,3,4
  for ( int i = 0; i < numT; i++ )
    state[ j++ ] = WB_dist_to_T[ i ]; // 5,6,7,8
  for ( int i = 1; i < numK; i++ )
    state[ j++ ] = dist_to_C_K[ i ]; // 9,10,11,12
  for ( int i = 0; i < numT; i++ )
    state[ j++ ] = dist_to_C_T[ i ]; // 13,14,15,16
  for ( int i = 1; i < numK; i++ )
    state[ j++ ] = nearest_Opp_dist_K[ i ]; // 17,18,19,20
  for ( int i = 1; i < numK; i++ )
    state[ j++ ] = nearest_Opp_ang_K[ i ]; // 21,22,23,24
*/
...
ArgumentationAgent::Situation ArgumentationAgent::getSituation(
        double state[]) {
    double minDist = state[5]; // the first one must be thhe closest
